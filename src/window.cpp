...
void ImGui_GL_NewFrame();
void ImGui_GL_RenderDrawData(ImDrawData* draw_data);
bool ImGui_GL_CreateFontsTexture();
void ImGui_GL_DestroyFontsTexture();
bool ImGui_GL_CreateDeviceObjects();
...
bool ImGui_GLFW_InitForOpenGL(GLFWwindow* window, bool install_callbacks);
bool ImGui_GLFW_InitForVulkan(GLFWwindow* window, bool install_callbacks);
void ImGui_GLFW_Shutdown();
void ImGui_GLFW_NewFrame();

static void ImGui_GL_SetupRenderState(ImDrawData* draw_data, int fb_width, int fb_height, GLuint vertex_array_object);
...
static bool CheckShader(GLuint handle, const char* desc)
{
    ...
    if ((GLboolean)status == GL_FALSE)
        fprintf(stderr, "ERROR: ImGui_GL_CreateDeviceObjects: failed to compile %s!\n", desc);
    if (log_length > 0)
    {
        ImVector<char> buf;
...
void ImGui_GLFW_NewFrame()
{
    ...
    ImGuiIO& io = ImGui::GetIO();
    IM_ASSERT(io.Fonts->IsBuilt() && "Font atlas not built! It is generally built by the renderer back-end. Missing call to renderer _NewFrame() function? e.g. ImGui_GL_NewFrame().");
